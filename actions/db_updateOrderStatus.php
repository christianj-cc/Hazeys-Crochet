...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['order_id']) && isset($_POST['order_status'])) {
        ...
        // Check if the current status exists in the flow
        if (isset($statusFlow[$order_status])) {
            ...
            if ($stmt->execute()) {
                $conn->commit(); // Commit changes if both queries succeed

                // Set success message in session
                $_SESSION['alert_message'] = "Order status updated successfully!";
                $_SESSION['alert_type'] = "success"; // Can be 'error' for failures

                header("Location: " . $return_url . "?success=Order-status-updated");
                exit();
            } else {
                $conn->rollback(); // Rollback if update fails

                // Set success message in session
                $_SESSION['alert_message'] = "Order status update failed!";
                $_SESSION['alert_type'] = "error"; // Can be 'error' for failures

                header("Location: " . $return_url . "?error=Order-status-update-failed");
                exit();
            }
